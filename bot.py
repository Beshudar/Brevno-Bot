...
def get_text_messages(message):
    if message.chat.id in admin :
        ...
        elif message.text == "/beep":
            os.system('beep')
            bot.send_message(message.from_user.id, "Биип выполнен!")
        ...
        #else : bot.send_message(message.from_user.id, f"Нету команды  {message.text}!") На доработку!
    else : 
        bot.send_message(message.from_user.id, "У вас нет прав на выполнение этой команды!")
        logging.error( f"Нет прав администратора для команд у - {message.from_user.id}" )
@bot.message_handler(content_types=['document'])#Получаем документы
def handle_docs_photo(message):
    if message.chat.id in admin :
        try:
            ...
            src = '/home/rimuru/documents/' + message.document.file_name;
        ...
        print(Back.GREEN + "[INFO]Сохранил изображение от @",message.from_user.username)
        print(Style.RESET_ALL)
    else :
        bot.reply_to(message, "Я не могу сохранить так как вы не администратор!")
